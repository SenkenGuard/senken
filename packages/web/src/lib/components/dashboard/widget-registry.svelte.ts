 ...
 * below. `$state`, not a plain module variable: `rendererFor`'s result is
 * read from inside `dashboard-grid.svelte`'s `{#each}` block, and a plain
 * variable read there creates no dependency at all — disabling a plugin
 * would then only ever show as a placeholder after something else forced
 * that block to re-run (in practice, only a full reload), never live. This
 * module reassigns the whole `Map` on every `registerPluginWidgets` call
 * rather than mutating one in place, which is exactly the shape `$state`
 * needs to notice a change — no `SvelteMap` or other deep-reactive
 * container required. */
let pluginWidgets = $state(new Map<string, WidgetPluginDefinition>());
 ...
 * `widget-catalog.svelte.ts`'s own `refreshWidgetCatalog`. Disabling a
 * plugin's package removes its entries from the *next* catalog fetch,
 * which is what makes a placed instance of one of its widgets fall back to
 * `rendererFor` returning `undefined` (a placeholder) the moment that
 * fetch resolves — live, no reload — the exact same fallback path an
 * uninstalled built-in already uses. */
