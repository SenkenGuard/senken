...
pub struct IndicatorDescriptor {
    ...
    /// Maximum non-series display objects (levels, segments, boxes, labels)
    /// this indicator may retain at once, oldest discarded first.
    ///
    /// A calculated series is exempt — it is one drawable regardless of its
    /// point count. This bound exists so a single mis-written indicator
    /// cannot retain unbounded memory on a server shared by many users; it
    /// is declared per indicator, not as one workspace-wide constant,
    /// because a future indicator that legitimately draws many zones should
    /// not have to share a budget sized for one that draws a handful.
