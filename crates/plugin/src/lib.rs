...
//!
//! Five capabilities exist today: [`MarketDataSource`] (instruments),
//! [`BarSource`] (bars), [`BookSource`] (order-book depth),
//! [`FeedSource`] (a live price/quote stream) and [`TradeAdapter`]
//! (trading). A plugin registering more than one of them for a single venue
//! must share one [`LimitGroup`] across all of them (obtain it once from an
...
mod tests {
    ...
    #[test]
    fn the_five_capabilities_drain_independently() {
