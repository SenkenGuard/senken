...
/// A watchlist group's primary key.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct WatchlistGroupId(Uuid);

/// One instrument's membership in a watchlist group's primary key.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct WatchlistMemberId(Uuid);
...
uuid_id!(WatchlistGroupId);
uuid_id!(WatchlistMemberId);

#[cfg(test)]
mod tests {
    use super::{WatchlistGroupId, WatchlistMemberId};

    #[test]
    fn two_freshly_generated_ids_of_each_kind_differ() {
        assert_ne!(WatchlistGroupId::new(), WatchlistGroupId::new());
        assert_ne!(WatchlistMemberId::new(), WatchlistMemberId::new());
    }

    #[test]
    fn display_round_trips_through_uuid_parsing() {
        let id = WatchlistGroupId::new();
