//! Compiles the indicator fixtures under `tests/fixtures/*` to real
//! `wasm32-wasip2` components on demand, so `dynamic_indicators.rs` proves
//! `senken_runtime::DynamicIndicators` against genuine compiled bytes
//! rather than a description of what a plugin would do.
//!
//! Mirrors `crates/plugin-host/tests/support/mod.rs` exactly, including its
//! process-wide build lock: this machine must never run two Rust builds at
//! once, and `cargo test`'s default parallel harness would otherwise start
//! one `cargo build` per fixture-dependent test function at the same time.
