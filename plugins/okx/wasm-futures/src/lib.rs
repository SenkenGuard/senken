//! OKX's dated-futures `venue-plugin` component: futures instruments and
...
struct OkxFuturesVenue;

impl VenueGuest for OkxFuturesVenue {
    fn descriptor() -> VenueDescriptor {
        VenueDescriptor {
            ...
            id: "okx-futures".to_owned(),
            name: "OKX Futures".to_owned(),
    ...
    fn instruments() -> Result<Vec<Instrument>, VenueError> {
        let body =
            fetch("/api/v5/public/instruments?instType=FUTURES", 1).map_err(VenueError::Fetch)?;
        let instruments = okx_core::parse_instruments(&body, okx_core::Market::Futures)
            .map_err(to_venue_error)?;
