//! OKX's `ETH-USD` options `venue-plugin` component: the instrument
...
struct OkxOptionEthUsdVenue;

impl VenueGuest for OkxOptionEthUsdVenue {
    fn descriptor() -> VenueDescriptor {
        VenueDescriptor {
            ...
            // change.
            id: "okx-option-eth-usd".to_owned(),
            name: "OKX ETH-USD Options".to_owned(),
    ...
    fn instruments() -> Result<Vec<Instrument>, VenueError> {
        let body = fetch(
            "/api/v5/public/instruments?instType=OPTION&instFamily=ETH-USD",
            1,
        )
...
export_venue!(OkxOptionEthUsdVenue);
